"""Gmail API client for reading medical emails."""
    ...
    def wrapper(*args, **kwargs):
        ...
        for attempt in range(_MAX_RETRIES):
            try:
                ...
                logger.warning(
                    "Gmail API %s failed (%s), retry %d/%d in %.1fs",
                    func.__name__,
                    str(e)[:120],
                    attempt + 1,
                    _MAX_RETRIES,
...
class GmailClient:
    """Read-only Gmail API client for scanning medical emails."""
    ...
    def from_oauth(
        ...
        """Create a GmailClient from OAuth 2.0 user credentials."""
        ...
        instance._service = build("gmail", "v1", credentials=creds)
        return instance
    ...
    def get_attachment(self, message_id: str, attachment_id: str) -> bytes:
        """Download an attachment. Returns decoded bytes."""
        result = (
            self._service.users()
            .messages()
            .attachments()
