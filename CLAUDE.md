...
- **Turso migrations**: NEVER use `ALTER TABLE RENAME` — Turso's Hrana protocol silently fails. Use `DROP TABLE IF EXISTS` + `CREATE TABLE` instead. Never INSERT patient-specific data in migrations (runs in test DBs). Use `WHERE EXISTS (SELECT 1 FROM patients WHERE ...)` guard if seeding data.
- **Date validation**: `_row_to_document()` in `_converters.py` uses bare `date.fromisoformat()` — a single invalid date string in the DB crashes the entire patient's document list (#258). AI-extracted dates can be hallucinated (e.g., `"2222-14-81"`). Always validate dates before writing to DB.
