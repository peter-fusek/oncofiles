...
- **Option A `patient_slug` rollout (#429)**: Patient-scoped MCP tools must accept `patient_slug: str | None = None` and resolve via `await _resolve_patient_id(patient_slug, ctx)` — NOT `_get_patient_id()` directly. This is the ONLY way the static `MCP_BEARER_TOKEN` can target a non-default patient in stateless HTTP (needed for cross-patient admin operations like backfills on specific patients). Wired on: `get_patient_context`, `update_patient_context`, `rename_documents_to_standard`, `enhance_documents`, `extract_all_metadata`, `backfill_ai_classification`, `detect_and_split_documents`, `detect_and_consolidate_documents`. Remaining ~40 tools need the same treatment (tracked in #429 carry-over).
