...
async def test_insert_document_with_version(db: Database):
    ...
    v1 = await db.insert_document(make_doc(file_id="f1"))
    v2 = await db.insert_document(make_doc(file_id="f2", version=2, previous_version_id=v1.id))
...
async def test_get_active_document_by_filename(db: Database):
    ...
    doc = await db.insert_document(make_doc(file_id="f1", original_filename="labs.pdf"))
    result = await db.get_active_document_by_filename("labs.pdf")
...
async def test_get_active_document_by_filename_returns_highest_version(db: Database):
    ...
    await db.insert_document(make_doc(file_id="f1", original_filename="labs.pdf", version=1))
    v2 = await db.insert_document(make_doc(file_id="f2", original_filename="labs.pdf", version=2))
...
async def test_get_active_document_by_filename_not_found(db: Database):
    ...
    result = await db.get_active_document_by_filename("nonexistent.pdf")
    assert result is None
...
async def test_version_chain_two_versions(db: Database):
    ...
    v1 = await db.insert_document(make_doc(file_id="f1", version=1))
    v2 = await db.insert_document(make_doc(file_id="f2", version=2, previous_version_id=v1.id))
...
async def test_version_chain_three_versions(db: Database):
    ...
    v1 = await db.insert_document(make_doc(file_id="f1", version=1))
    v2 = await db.insert_document(make_doc(file_id="f2", version=2, previous_version_id=v1.id))
    v3 = await db.insert_document(make_doc(file_id="f3", version=3, previous_version_id=v2.id))
