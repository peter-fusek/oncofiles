...
async def test_insert_document_with_version(db: Database):
    ...
    v1 = await db.insert_document(make_doc(file_id="f1"), patient_id="erika")
    v2 = await db.insert_document(
        make_doc(file_id="f2", version=2, previous_version_id=v1.id), patient_id="erika"
    )
...
async def test_get_active_document_by_filename(db: Database):
    ...
    doc = await db.insert_document(
        make_doc(file_id="f1", original_filename="labs.pdf"), patient_id="erika"
    )
    result = await db.get_active_document_by_filename("labs.pdf", patient_id="erika")
...
async def test_get_active_document_by_filename_returns_highest_version(db: Database):
    ...
    await db.insert_document(
        make_doc(file_id="f1", original_filename="labs.pdf", version=1), patient_id="erika"
    )
    v2 = await db.insert_document(
        make_doc(file_id="f2", original_filename="labs.pdf", version=2), patient_id="erika"
    )
...
async def test_get_active_document_by_filename_not_found(db: Database):
    ...
    result = await db.get_active_document_by_filename("nonexistent.pdf", patient_id="erika")
    assert result is None
...
async def test_version_chain_two_versions(db: Database):
    ...
    v1 = await db.insert_document(make_doc(file_id="f1", version=1), patient_id="erika")
    v2 = await db.insert_document(
        make_doc(file_id="f2", version=2, previous_version_id=v1.id), patient_id="erika"
    )
...
async def test_version_chain_three_versions(db: Database):
    ...
    v3 = await db.insert_document(
        make_doc(file_id="f3", version=3, previous_version_id=v2.id), patient_id="erika"
    )
