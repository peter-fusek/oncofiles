...
async def test_multi_term_and_semantics(db: Database):
    ...
    await db.insert_document(doc1, patient_id=ERIKA_UUID)
    await db.insert_document(doc2, patient_id=ERIKA_UUID)

    # "NOUonko CBC" → only doc1 matches both terms
    results = await db.search_documents(SearchQuery(text="NOUonko CBC"), patient_id=ERIKA_UUID)
    assert len(results) == 1
    assert results[0].file_id == "f1"

    # "labs CBC" → both match
    results = await db.search_documents(SearchQuery(text="labs CBC"), patient_id=ERIKA_UUID)
...
async def test_relevance_filename_over_summary(db: Database):
    ...
    results = await db.search_documents(SearchQuery(text="CEA"), patient_id=ERIKA_UUID)
    assert len(results) == 2
...
async def test_relevance_summary_over_tags(db: Database):
    ...
    results = await db.search_documents(SearchQuery(text="FOLFOX"), patient_id=ERIKA_UUID)
    assert len(results) == 2
...
async def test_search_structured_metadata(db: Database):
    ...
    results = await db.search_documents(SearchQuery(text="EGFR"), patient_id=ERIKA_UUID)
    assert len(results) == 1
    assert results[0].file_id == "f1"
...
async def test_search_offset_pagination(db: Database):
    ...
    page1 = await db.search_documents(
        SearchQuery(text="CBC", limit=2, offset=0), patient_id=ERIKA_UUID
    )
    assert len(page1) == 2

    # Page 2: next 2
    page2 = await db.search_documents(
        SearchQuery(text="CBC", limit=2, offset=2), patient_id=ERIKA_UUID
    )
    assert len(page2) == 2
    ...
    page3 = await db.search_documents(
        SearchQuery(text="CBC", limit=2, offset=4), patient_id=ERIKA_UUID
    )
    assert len(page3) == 1
...
async def test_search_no_text_date_order(db: Database):
    ...
    results = await db.search_documents(SearchQuery(category="labs"), patient_id=ERIKA_UUID)
    assert len(results) == 2
...
async def test_search_empty_query_returns_all(db: Database):
    ...
    results = await db.search_documents(SearchQuery(), patient_id=ERIKA_UUID)
    assert len(results) == 2
