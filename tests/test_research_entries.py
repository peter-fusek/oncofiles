...
from .helpers import ERIKA_UUID, make_research_entry
...
async def test_dedup_by_source_external_id(db: Database):
    ...
    saved1 = await db.insert_research_entry(e1, patient_id=ERIKA_UUID)
    saved2 = await db.insert_research_entry(e2, patient_id=ERIKA_UUID)
...
async def test_search_by_text_in_title(db: Database):
    ...
    results = await db.search_research_entries(ResearchQuery(text="FOLFOX"), patient_id=ERIKA_UUID)
...
async def test_search_by_text_in_summary(db: Database):
    ...
    results = await db.search_research_entries(
        ResearchQuery(text="oxaliplatin"), patient_id=ERIKA_UUID
    )
    assert len(results) == 1
...
async def test_search_by_text_in_tags(db: Database):
    ...
    results = await db.search_research_entries(ResearchQuery(text="mCRC"), patient_id=ERIKA_UUID)
    assert len(results) == 1


async def test_search_by_source(db: Database):
    await db.insert_research_entry(
        make_research_entry(source="pubmed", external_id="1"), patient_id=ERIKA_UUID
    )
    await db.insert_research_entry(
        make_research_entry(source="clinicaltrials", external_id="2"), patient_id=ERIKA_UUID
    )

    results = await db.search_research_entries(
        ResearchQuery(source="pubmed"), patient_id=ERIKA_UUID
    )
    assert len(results) == 1
...
async def test_search_combined_text_and_source(db: Database):
    ...
    results = await db.search_research_entries(
        ResearchQuery(text="FOLFOX", source="clinicaltrials"), patient_id=ERIKA_UUID
    )
    assert len(results) == 1
...
async def test_list_all(db: Database):
    await db.insert_research_entry(make_research_entry(external_id="1"), patient_id=ERIKA_UUID)
    await db.insert_research_entry(make_research_entry(external_id="2"), patient_id=ERIKA_UUID)

    entries = await db.list_research_entries(patient_id=ERIKA_UUID)
...
async def test_list_by_source(db: Database):
    ...
    entries = await db.list_research_entries(source="pubmed", patient_id=ERIKA_UUID)
...
async def test_list_with_limit(db: Database):
    for i in range(5):
        await db.insert_research_entry(
            make_research_entry(external_id=f"PM{i}"), patient_id=ERIKA_UUID
        )

    entries = await db.list_research_entries(limit=3, patient_id=ERIKA_UUID)
    assert len(entries) == 3


async def test_timestamps_set(db: Database):
    saved = await db.insert_research_entry(make_research_entry(), patient_id=ERIKA_UUID)
