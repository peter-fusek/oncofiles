...
async def test_insert_and_get(db: Database):
    entry = make_entry()
    inserted = await db.insert_conversation_entry(entry)
...
async def test_delete(db: Database):
    entry = await db.insert_conversation_entry(make_entry())
...
async def test_search_fts(db: Database):
    ...
    results = await db.search_conversation_entries(ConversationQuery(text="FOLFOX"))
    assert len(results) == 1
    assert "FOLFOX" in results[0].title


async def test_search_by_entry_type(db: Database):
    await db.insert_conversation_entry(make_entry(entry_type="decision"))
    await db.insert_conversation_entry(make_entry(entry_type="note"))

    results = await db.search_conversation_entries(ConversationQuery(entry_type="decision"))
    assert len(results) == 1
    assert results[0].entry_type == "decision"


async def test_search_by_participant(db: Database):
    await db.insert_conversation_entry(make_entry(participant="claude-code"))
    await db.insert_conversation_entry(make_entry(participant="claude.ai"))

    results = await db.search_conversation_entries(ConversationQuery(participant="claude-code"))
    assert len(results) == 1
    assert results[0].participant == "claude-code"


async def test_search_by_date_range(db: Database):
    await db.insert_conversation_entry(make_entry(entry_date=date(2025, 1, 10)))
    await db.insert_conversation_entry(make_entry(entry_date=date(2025, 2, 15)))
    await db.insert_conversation_entry(make_entry(entry_date=date(2025, 3, 20)))
...
async def test_search_by_tags(db: Database):
    await db.insert_conversation_entry(make_entry(tags=["chemo", "FOLFOX"]))
    await db.insert_conversation_entry(make_entry(tags=["labs", "blood"]))

    results = await db.search_conversation_entries(ConversationQuery(tags=["FOLFOX"]))
    assert len(results) == 1
...
async def test_timeline_chronological_order(db: Database):
    await db.insert_conversation_entry(make_entry(entry_date=date(2025, 3, 1), title="March"))
    await db.insert_conversation_entry(make_entry(entry_date=date(2025, 1, 1), title="January"))
    await db.insert_conversation_entry(make_entry(entry_date=date(2025, 2, 1), title="February"))
...
async def test_timeline_with_date_range(db: Database):
    await db.insert_conversation_entry(make_entry(entry_date=date(2025, 1, 1)))
    await db.insert_conversation_entry(make_entry(entry_date=date(2025, 6, 1)))
...
async def test_source_ref_idempotency(db: Database):
    ...
    await db.insert_conversation_entry(entry)

    found = await db.get_entry_by_source_ref("session_abc.jsonl")
    assert found is not None
    assert found.source_ref == "session_abc.jsonl"


async def test_source_ref_not_found(db: Database):
    result = await db.get_entry_by_source_ref("nonexistent.jsonl")
    assert result is None
