...
async def test_find_duplicates_none(db: Database):
    ...
    await db.insert_document(make_doc(file_id="f1", original_filename="a.pdf"))
    await db.insert_document(make_doc(file_id="f2", original_filename="b.pdf"))
...
async def test_purge_expired_trash_none(db: Database):
    ...
    count = await db.purge_expired_trash(days=30)
    assert count == 0
...
async def test_purge_cleans_ocr_pages(db: Database):
    ...
    await db.purge_expired_trash(days=30)
...
async def test_purge_preserves_active_documents(db: Database):
    ...
    count = await db.purge_expired_trash(days=0)  # even with 0 days
    assert count == 0
