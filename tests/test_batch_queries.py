...
async def test_get_documents_by_ids_multiple(db):
    ...
    d1 = await db.insert_document(make_doc(file_id="f1", filename="a.pdf"))
    d2 = await db.insert_document(make_doc(file_id="f2", filename="b.pdf"))
    d3 = await db.insert_document(make_doc(file_id="f3", filename="c.pdf"))
...
async def test_get_previous_lab_values_single_entry(db):
    ...
    d1 = await db.insert_document(make_doc(file_id="f1"))
    v1 = LabValue(
...
async def test_get_previous_lab_values_two_entries(db):
    ...
    d1 = await db.insert_document(make_doc(file_id="f1"))
    d2 = await db.insert_document(make_doc(file_id="f2"))
