...
async def test_setup_gdrive_creates_all_folders(db: Database):
    ...
    assert result["total_folders"] == 20  # 17 categories + 3 metadata
    assert len(result["created"]) == 20
    assert len(result["skipped"]) == 0
    assert len(result["renamed"]) == 0
    assert mock_gdrive.create_folder.call_count == 20
...
async def test_setup_gdrive_renames_old_folders(db: Database):
    ...
    assert result["status"] == "ok"
    assert len(result["renamed"]) == 20
    assert len(result["created"]) == 0
    assert mock_gdrive.rename_file.call_count == 20
...
async def test_setup_gdrive_idempotent_mixed(db: Database):
    ...
    assert len(result["created"]) == 19  # Rest were created
